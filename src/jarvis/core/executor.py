...
class Executor:
    """
    Control Execution Engine for Jarvis do Cerrado.

    This class serves as the central hub for executing actions based on user intents.
    It enforces security policies (like confirmation for sensitive actions),
    logs events, and dispatches commands to specific modules (Network, System, Hydration).

    Attributes:
        app: The Telegram Application instance.
        pending_actions (Dict): Storage for actions requiring user confirmation.
    """
    ...
    async def execute(self, intent_data: Dict[str, Any], chat_id: int) -> str:
        """
        Executes the logic associated with a given intent.

        This method handles:
        1. Validation of the intent data.
        2. Logging and Context saving.
        3. Handling of confirmation flows (action_confirm/cancel).
        4. Enforcement of security checks for sensitive actions.
        5. Dispatching to specific handlers.

        Args:
            intent_data (Dict[str, Any]): The intent dictionary (e.g., {'intent': 'help', ...}).
            chat_id (int): The Telegram Chat ID of the user.

        Returns:
            str: The response text to be sent back to the user (or a Dict with reply_markup).
        """
    ...
    async def _execute_intent(self, intent: str, action: str, params: Dict[str, Any], chat_id: int) -> str:
        """
        Internal dispatcher for intents.

        Args:
            intent (str): The intent name.
            action (str): The specific action within the intent.
            params (Dict): Parameters extracted from the user input.
    ...
    async def _confirm_action(self, chat_id: int) -> str:
        """Confirms a pending sensitive action."""
    ...
    async def handle_network_callback(self, chat_id: int, data: str, query):
        """
        Handles callbacks generated by network events (like new device detection).
