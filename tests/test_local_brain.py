...
class TestBrainLocalFallback(unittest.IsolatedAsyncioTestCase):
    async def test_brain_local_fallback(self):
        ...
        self.assertIn("ROD do Cerrado", result["params"]["response"])
        ...
        self.assertIn("ROD", result_fuzzy["params"]["response"])
        ...
        # Should be fallback
        self.assertNotIn("ROD do Cerrado", result_unknown["params"]["response"])
